...
This image is automatically built for versions 8.1.13-8.1.27, new versions will be updated, but any features are subject to change with later versions. Upon a new pull request, if a valid build file is modified, it will trigger a build test pipeline that verifies the image still operates as expected.
...
When pulling the docker image, note that using the copy link from the home page (`docker pull bwdesigngroup/ignition-docker`) will automatically pull the most recent version of Ignition configured in the image. For example `:latest` may pull version `8.1.27` as of the time of writing.
...
### Example docker-compose file
	...
	services:
	  gateway:
		  image: bwdesigngroup/ignition-docker:8.1.27
		  ports:
		  - 80:8088
